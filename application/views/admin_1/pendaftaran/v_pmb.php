<main id="main" class="main">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-1">
                                    <?php
                                    if ($s->status == '0') {
                                        echo "<a data-bs-toggle='modal' data-bs-target='#konfirm$s->id_pmb_karantina' href='#'
                                        class='btn btn-secondary btn-sm'>Konfirm</a>";
                                    } else {
                                        echo "<a href=" . base_url('admin_satu/detail_pmb/' . $s->id_pmb_karantina) . " 
                                    ...
                                    class='btn btn-success btn-sm'><i class='bi bi-whatsapp'></i></a>";
                                        echo "<a id='hapus_karantina' data-id='$s->id_pmb_karantina' class='btn btn-danger btn-sm'><i class='bi bi-trash3-fill'></i></a>";
                                    }
                                    ?>
                                </div>
                            </td>
    ...
    <?php foreach ($pmb as $s) { ?>
        <!-- Konfirmasi -->
        <div class="modal fade" id="konfirm<?= $s->id_pmb_karantina; ?>" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="<?php echo base_url('admin_satu/konfirmasi/' . $s->id_pmb_karantina) ?>">
                            <div class=" mb-2">
                                <label for="exampleFormControlInput1" class="form-label bold">Pilihan
                                    Program :</label>
                                <input type="text" class="form-control" id="exampleFormControlInput1"
                                    value="<?= $s->prog ?>" disabled>
                            </div>
                            <div class="mb-2">
        ...
        <!-- Bukti TF -->
        <div class="modal fade" id="bukti_tf<?= $s->id_pmb_karantina; ?>" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header head">
                    ...
                    <div class="modal-body">
                        <div class="d-flex justify-content-center">
                            <img src="<?= base_url() ?>assets/backend/upload/karantina_dauroh/<?= $s->bukti_tf ?>"
                                width="240px">
                        </div>
                    </div>
                    <div class="modal-footer">
        ...
        <!-- hapus_pmb -->
        <div class="modal fade" id="hapus<?= $s->id_pmb_karantina; ?>" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-sm">
                <div class="modal-content">
                    <div class="modal-header head">
                        <h1 class="modal-title fs-5 bold" id="exampleModalLabel">Yakin ingin hapus data ini ?</h1>
                    </div>
                    <div class="modal-body">
                        <form action="<?php echo base_url('admin_satu/hapus_pmb/' . $s->id_pmb_karantina) ?>" method="POST">
                            <div class=" mb-2">
                                <label for="exampleFormControlInput1" class="form-label bold">Pilihan
                                    Program :</label>
                                <input type="hidden" name="foto" value="<?= $s->foto ?>">
                                <input type="hidden" name="kk" value="<?= $s->kk ?>">
                                <input type="hidden" name="ijazah" value="<?= $s->ijazah ?>">
                                <input type="hidden" name="tes" value="<?= $s->tes ?>">
                                <input type="hidden" name="bukti_tf" value="<?= $s->bukti_tf ?>">
                                <input type="text" class="form-control" id="exampleFormControlInput1"
                                    value="<?= $s->prog ?>" disabled>
                            </div>
                            <div class="mb-2">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Kembali</button>
                        <button type="submit" class="btn btn-danger btn-sm bold">Ya Hapus!</button>
                    </div>
                    </form>
                </div>
            </div>
